...
def run(force_cross_validate: bool = False, use_resample: bool = True):
    ...
    if force_cross_validate:
        xgb_results = cross_validate_model(ModelType.XGB, XGBConfig(max_depth=5), X_cv.values, y_cv.values)
        rf_results = cross_validate_model(ModelType.RF, RFConfig(max_depth=5), X_cv.values, y_cv.values)
